                    ...
                    <table id="data-table" class="table data-table mb-0">
                        ...
                        <tbody>
                           @forelse ($slider as $slider)
                            <tr>
                                <td>{{$loop -> index + 1}}</td>
                                <td style="width: 100px;">{{$slider -> title}}</td>
                                
                                <td>
                                    <img style="height: 50px; width: 50px;" src="{{url('storage/slider/'. $slider -> photo )}}" alt="">
                                </td>
                                <td>{{$slider -> created_at -> diffForHumans()}}</td>
                                <td>
                                    @if ($slider -> status)
                                       <span class="badge badge-success">Active user</span>
                                       <a class=" btn text-danger" href="{{route('slider.status.update', $slider -> id)}}"><i class="fa fa-times"></i></a>
                                       @else
                                       <span class="badge badge-danger">Block user</span>
                                       <a class=" btn btn-xl text-primary" href="{{route('slider.status.update', $slider-> id)}}"><i class="fa fa-check"></i></a>
                                ...
                                <td>
                                    ...
                                    <a class=" btn btn-sm btn-primary" href="{{route('slider.edit', $slider-> id)}}"><i class="fa fa-edit"></i></a>
                                     
                                   <form class="d-inline delete-btn" action="{{route('slider.destroy', $slider -> id)}}" method="POST">
                                        @csrf
                                        @method('DELETE')
